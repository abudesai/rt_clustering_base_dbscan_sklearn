...
def save_scoring_outputs(results, dataset_name, run_hpt=False, chart_data=None):
    ...
    if chart_data is not None:
        ...
        file_path_and_name = get_file_path_and_name(
            dataset_name, run_hpt, file_type="scatter"
        )
        plt.suptitle(
            f"clusters on tsne-reduced data for {dataset_name} dataset with {run_hpt=}"
        )
...
if __name__ == "__main__":
    ...
    for run_hpt in run_hpt_list:
        ...
        for dataset_name in datasets:
            ...
            results, chart_data = run_train_and_test(
